...
class Q_Map_DQN_Agent:
    def __init__(self,
        ...
        if q_map_model is not None:
            agent_name += 'qmap'#'-Q-MAP-' + q_map_model.description[:13] + '-' + str(q_map_min_goal_steps) + '-' + str(q_map_max_goal_steps) + '-gamma' + str(q_map_gamma) + '-lr' + str(q_map_lr) + '-bias' + str(q_map_greedy_bias) + '-bonus' + str(q_map_timer_bonus)
            self.use_q_map = True
        ...
        self.path = '{}/{}/{}/{}'.format(path, 'CustomSuperMarioAllStars', agent_name, sub_name)
    ...
    def optimize(self):
        ...
        # save the session
        if (self.use_dqn or self.use_q_map) and (self.t+1) % 50000 == 0:
