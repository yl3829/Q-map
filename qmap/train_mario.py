...
parser.add_argument('--nsteps',help='steps to train',type=int, default=5e6)
...
for t in range(n_steps+1):
    if done:
        ...
        if episode > 0 and (episode < 10 or episode % 10 == 0 or new_best):
            current_time = time.time()
            ips = (t - last_ips_t) / (current_time - previous_time)
            print('step: {} IPS: {:.2f}'.format(t+1, ips))
            name = 'score_%08.3f'%score + '_d_' + str(distance) + '_s_' + str(t+1) + '_ep_' + str(episode)
